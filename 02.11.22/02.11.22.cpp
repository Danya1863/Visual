...
{
    TCHAR buf11[100] = TEXT("Данил");
    TCHAR buf12[100] = TEXT("Ковалёв");
    TCHAR buf13[100] = TEXT("23.12.2003");
    TCHAR buf21[100] = TEXT("Денис");
    TCHAR buf22[100] = TEXT("Багацкий");
    TCHAR buf23[100] = TEXT("11.05.2005");
    switch (message)
    {
        /*case WM_MOUSEMOVE:
    ...
    case WM_LBUTTONDOWN:
    {
        int task = MessageBox(hWnd, TEXT("OK- Резюме\nCancel - Отгадывание чисел"), TEXT("Boomer/Slash"), MB_OKCANCEL | MB_ICONASTERISK);
        if(task == IDOK)
        {
            int rez = MessageBox(hWnd, TEXT("YES - 1 резюме\nNO - 2 резюме\nCancel - Среднее арифмитичское символов"), TEXT("Резюмe"), MB_YESNOCANCEL | MB_ICONASTERISK);
            if (rez == IDYES)
            {
                MessageBox(hWnd, buf11, TEXT("Name"), MB_OK);
                MessageBox(hWnd, buf12, TEXT("Surname"), MB_OK);
                MessageBox(hWnd, buf13, TEXT("Birthday"), MB_OK);
            }
            else if (rez == IDCANCEL)
            {
                ...
                MessageBox(hWnd, buf, TEXT("C plus plus"), MB_OK);
            }
            else if (rez == IDNO)
            {
                MessageBox(hWnd, buf21, TEXT("Name"), MB_OK);
                MessageBox(hWnd, buf22, TEXT("Surname"), MB_OK);
                MessageBox(hWnd, buf23, TEXT("Birthday"), MB_OK);
            }
            break;
        }
        else if (task == IDCANCEL)
        {
            ...
            {
                TCHAR buf[50];
                wsprintf(buf, TEXT("Your number %d?"), temp);
